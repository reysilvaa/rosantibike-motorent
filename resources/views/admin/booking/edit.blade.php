...
<div class="container mx-auto p-4 sm:p-6 lg:p-10 bg-gray-100 rounded-lg shadow-lg">
    <h2 class="text-3xl sm:text-4xl font-bold mb-6 sm:mb-10 text-gray-800 text-center">Edit Booking</h2>
    ...
    <form action="{{ route('admin.booking.update', $booking->id) }}" method="POST" class="bg-white p-6 sm:p-8 rounded-lg shadow-lg space-y-6 sm:space-y-8" id="editBookingForm">
        @csrf
        @method('PUT')

        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8">
            <div class="space-y-4 sm:space-y-6">
                @foreach(['id' => 'ID Booking', 'id_user' => 'ID User', 'nama_penyewa' => 'Nama Penyewa'] as $field => $label)
                    <div class="form-group">
                        <label for="{{ $field }}" class="block text-sm font-medium text-gray-700 mb-1">{{ $label }}</label>
                        <input type="text" id="{{ $field }}" name="{{ $field }}" value="{{ $booking->$field }}" class="w-full px-3 py-2 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-transparent bg-gray-100" disabled>
                    </div>
                @endforeach
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8 mt-6">
            <div class="form-group">
                ...
                <input type="date" id="tgl_sewa" name="tgl_sewa" value="{{ $booking->tgl_sewa->format('Y-m-d') }}" class="w-full px-3 py-2 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-transparent bg-gray-100" disabled>
            </div>

            <div class="form-group">
                <label for="tgl_kembali" class="block text-sm font-medium text-gray-700 mb-1">Tanggal Kembali</label>
                <input type="date" id="tgl_kembali" name="tgl_kembali" value="{{ $booking->tgl_kembali->format('Y-m-d') }}" class="w-full px-3 py-2 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-transparent @error('tgl_kembali') border-red-500 @enderror">
                @error('tgl_kembali')
        ...
        <div class="mt-8 sm:mt-12 bg-gray-50 p-4 sm:p-6 rounded-lg shadow-inner">
            ...
            <input type="hidden" id="id_jenis" name="id_jenis" value="{{ $booking->id_jenis }}">
            @error('id_jenis')
        ...
        <div class="mt-8 sm:mt-12 bg-white p-4 sm:p-6 rounded-lg shadow-md">
            ...
            <input type="hidden" id="total" name="total" value="{{ $booking->total }}">
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const originalTotal = parseFloat("{{ $booking->total }}");
    const originalMotor = "{{ $booking->jenisMotor->stok->merk }} (Nopol: {{ $booking->jenisMotor->nopol }})";
    ...
    function updateTotal() {
        const originalTglKembali = new Date("{{ $booking->tgl_kembali->format('Y-m-d') }}");
