...
<div class="container mx-auto p-10 bg-gray-100 rounded-lg shadow-lg">
    <h2 class="text-4xl font-bold mb-10 text-gray-800 text-center">Edit Booking</h2>
    ...
    <form action="{{ route('admin.booking.update', $booking->id) }}" method="POST" class="bg-white p-8 rounded-lg shadow-lg space-y-8" id="editbookingForm">
        @csrf
        @method('PUT')
            ...
            <div class="space-y-6">
                @foreach(['id' => 'ID Booking', 'id_user' => 'ID User', 'nama_penyewa' => 'Nama Penyewa'] as $field => $label)
                    <div class="form-group">
                        ...
                        <input type="text" id="{{ $field }}" name="{{ $field }}" value="{{ $booking->$field }}" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-transparent bg-gray-100" disabled>
                    </div>
                @endforeach
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-12 mt-8">
            <div class="form-group">
                ...
                <input type="date" id="tgl_sewa" name="tgl_sewa" value="{{ $booking->tgl_sewa->format('Y-m-d') }}" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-transparent bg-gray-100" disabled>
            </div>

            <div class="form-group">
                <label for="tgl_kembali" class="block text-sm font-medium text-gray-700 mb-2">Tanggal Kembali</label>
                <input type="date" id="tgl_kembali" name="tgl_kembali" value="{{ $booking->tgl_kembali->format('Y-m-d') }}" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-transparent @error('tgl_kembali') border-red-500 @enderror">
                @error('tgl_kembali')
