    ...
    <div class="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden">
        <div class="bg-gradient-to-r from-blue-500 to-indigo-600 p-6 sm:p-10">
            ...
            <p class="mt-2 text-blue-100">
                Update details for {{ $jenisMotor->stok->merk }}
            </p>
        </div>

        <form action="{{ route('admin.jenisMotor.update', $jenisMotor->id) }}"
              method="POST"
                ...
                <div>
                    ...
                    <select name="id_stok" id="id_stok"
                        ...
                        @foreach($stoks as $stok)
                            <option value="{{ $stok->id }}" {{ old('id_stok', $jenisMotor->stok->id ?? '') == $stok->id ? 'selected' : '' }}>
                                {{ $stok->merk }}
                            </option>
                        @endforeach
                    </select>
