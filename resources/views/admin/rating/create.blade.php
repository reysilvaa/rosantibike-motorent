...
<div class="container mx-auto px-4 py-8">
    ...
    <form action="{{ route('admin.rating.store') }}" method="POST" enctype="multipart/form-data" class="bg-white shadow-lg rounded-lg p-8 space-y-6">
        @csrf

        <!-- Nama Field -->
        <div class="flex flex-col space-y-2">
            <label for="nama" class="text-sm font-medium text-gray-700">Name</label>
            <input type="text" id="nama" name="nama" value="{{ old('nama') }}" class="w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" required>
            @error('nama')
        ...
        <div class="flex flex-col space-y-2">
            ...
            <input type="text" id="role" name="role" value="{{ old('role') }}" class="w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" required>
            @error('role')
                <p class="text-red-500 text-xs">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="flex flex-col space-y-2">
            ...
            <textarea id="deskripsi" name="deskripsi" class="w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" required>{{ old('deskripsi') }}</textarea>
            @error('deskripsi')
                <p class="text-red-500 text-xs">{{ $message }}</p>
            @enderror
        </div>
        ...
        <!-- Rating Field -->
        <div x-data="{ rating: {{ old('rating', 0) }} }" class="flex flex-col space-y-2">
        ...
        <div class="flex flex-col space-y-2">
            ...
            <input type="date" id="tanggal" name="tanggal" value="{{ old('tanggal') }}" class="w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" required>
            @error('tanggal')
                <p class="text-red-500 text-xs">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div x-data="{ avatarUrl: '{{ old('avatar_url') }}', avatarFile: null }" class="flex flex-col space-y-4">
            ...
            <div class="flex flex-col space-y-2">
                ...
                <!-- Avatar URL -->
                <input @input="avatarUrl = $event.target.value; avatarFile = null" type="url" id="avatar_url" name="avatar_url" value="{{ old('avatar_url') }}" placeholder="https://example.com/avatar.jpg" class="w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                @error('avatar_url')
                    <p class="text-red-500 text-xs">{{ $message }}</p>
                @enderror
            </div>
