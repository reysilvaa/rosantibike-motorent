...
<div class="min-h-screen bg-gradient-to-br from-blue-100 to-indigo-200 py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden">
        <div class="bg-gradient-to-r from-blue-500 to-indigo-600 p-6 sm:p-10">
            ...
            <p class="mt-2 text-blue-100">Detail Update untuk {{ $stok->merk }}</p>
        </div>

        <form action="{{ route('admin.stok.update', $stok->id) }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-10 space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="merk" id="merk" value="{{ old('merk', $stok->merk) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50 transition duration-300" required>
                        @error('merk')
                    ...
                    <div>
                        ...
                        <div class="mt-1 relative rounded-md shadow-sm">
                            ...
                            <input type="number" name="harga_perHari" id="harga_perHari" value="{{ old('harga_perHari', $stok->harga_perHari) }}" class="pl-10 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50 transition duration-300" required>
                        </div>
                        @error('harga_perHari')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="foto_url" id="foto_url" value="{{ old('foto_url', $stok->foto_url) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50 transition duration-300" placeholder="https://example.com/image.jpg">
            ...
            <div class="mt-8 border-t border-gray-200 pt-8">
                ...
                <div class="mt-4 bg-gray-100 p-4 rounded-lg">
                    <div id="currentPhoto" class="aspect-w-16 aspect-h-9">
                        @if ($stok->foto_url)
                            <img src="{{ $stok->foto_url }}" alt="{{ $stok->merk }}" class="w-full h-full object-cover rounded-lg shadow-md">
                        @elseif ($stok->foto)
                            <img src="{{ asset('storage/' . $stok->foto) }}" alt="{{ $stok->merk }}" class="w-full h-full object-cover rounded-lg shadow-md">
                        @else
